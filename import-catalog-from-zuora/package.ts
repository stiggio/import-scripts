import { isDryRun, environmentId, updateMode, customPlans } from "./arguments";
import {
...
function getCreatePackageInput(
  ...
  if (isDryRun) {
    return {
      input: {
        ...
        environmentId,
        pricingType: customPlans ? "CUSTOM" : "PAID",
        status: "DRAFT",
      },
    };
  }
  ...
  const additionalMetaData: Record<string, string> = {
    ...
    ...(customPlans ? { ZUORA__PLAN_IDS: zuoraPlanIds.join(",") } : {}),
  };
  ...
  return {
    input: {
      ...
      billingId: zuoraProductId,
      environmentId,
      pricingType: customPlans ? "CUSTOM" : isPaid ? "PAID" : "FREE",
      status: "DRAFT",
    },
  };
}
